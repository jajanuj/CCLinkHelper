...
namespace WindowsFormsApp1.CCLink.Adapters
{
   ...
   public sealed class MelsecApiAdapter : IMelsecApiAdapter
   {
      ...
      public short Open(short chan, short mode, out int path) => MelsecApi.mdOpen(chan, mode, out path);

      public short Close(int path) => MelsecApi.mdClose(path);

      public int SendEx(int path, int netNo, int stNo, int devType, int devNo, ref int size, short[] data) =>
         MelsecApi.mdSendEx(path, netNo, stNo, devType, devNo, ref size, data);

      public int ReceiveEx(int path, int netNo, int stNo, int devType, int devNo, ref int size, short[] data) =>
         MelsecApi.mdReceiveEx(path, netNo, stNo, devType, devNo, ref size, data);

      public int DevSetEx(int path, int netNo, int stNo, int devType, int devNo) =>
         MelsecApi.mdDevSetEx(path, netNo, stNo, devType, devNo);

      public int DevRstEx(int path, int netNo, int stNo, int devType, int devNo) =>
         MelsecApi.mdDevRstEx(path, netNo, stNo, devType, devNo);

      public int RandWEx(int path, int netNo, int stNo, int[] dev, short[] buf, int bufSize) =>
         MelsecApi.mdRandWEx(path, netNo, stNo, dev, buf, bufSize);

      public int RandREx(int path, int netNo, int stNo, int[] dev, short[] buf, int bufSize) =>
         MelsecApi.mdRandREx(path, netNo, stNo, dev, buf, bufSize);

      public int RemBufWriteEx(int path, int netNo, int stNo, int offset, ref int size, short[] data) =>
         MelsecApi.mdRemBufWriteEx(path, netNo, stNo, offset, ref size, data);

      public int RemBufReadEx(int path, int netNo, int stNo, int offset, ref int size, short[] data) =>
         MelsecApi.mdRemBufReadEx(path, netNo, stNo, offset, ref size, data);

      public int RemBufWriteIPEx(int path, int ipAddress, int offset, ref int size, short[] data) =>
         MelsecApi.mdRemBufWriteIPEx(path, ipAddress, offset, ref size, data);

      public int RemBufReadIPEx(int path, int ipAddress, int offset, ref int size, short[] data) =>
         MelsecApi.mdRemBufReadIPEx(path, ipAddress, offset, ref size, data);

      public short TypeRead(int path, short stNo, out short buf) => MelsecApi.mdTypeRead(path, stNo, out buf);

      public short Control(int path, short stNo, short buf) => MelsecApi.mdControl(path, stNo, buf);

      public short BoardRst(int path) => MelsecApi.mdBdRst(path);

      public short BoardModSet(int path, short mode) => MelsecApi.mdBdModSet(path, mode);

      public short BoardModRead(int path, out short mode) => MelsecApi.mdBdModRead(path, out mode);

      public short BoardLedRead(int path, short[] buf) => MelsecApi.mdBdLedRead(path, buf);

      public short BoardSwRead(int path, short[] buf) => MelsecApi.mdBdSwRead(path, buf);

      public short BoardVerRead(int path, short[] buf) => MelsecApi.mdBdVerRead(path, buf);

      public short Init(int path) => MelsecApi.mdInit(path);
